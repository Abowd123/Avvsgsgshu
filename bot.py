...
def _load_provider(
    blueprint: dict[str, Any], priority: int, *, override_key: str = ""
) -> ProviderConfig | None:
    """
...
def _build_providers(store: "KeyStore | None") -> tuple[ProviderConfig, ...]:
    ...
    return tuple(
        ...
        for provider in (
            _load_provider(
                blueprint,
                priority,
                override_key=store.get(blueprint["prefix"]) if store else "",
            )
...
class Settings:
    ...
    @classmethod
    def load(cls, store: "KeyStore | None" = None) -> "Settings":
        """
        Build the settings object from the process environment.

        `store` layers keys saved through the `/keys` admin panel on top of `.env`.
        A provider key is no longer required at startup: the bot can start with zero
        providers configured and an admin can add the first one from inside the bot
        (see `KeyStore`, `AIProviderManager.reload`) without a restart.
        """
        providers = _build_providers(store)
...
def render_key_detail(
    blueprint: dict[str, Any], configured: bool, masked: str, health_line: str = ""
) -> str:
...
async def on_keys_open(
    ...
    if isinstance(callback.message, Message):
        with suppress(TelegramAPIError):
            await callback.message.edit_text(
                render_key_detail(blueprint, configured, masked, health_line),
...
async def on_keys_test(
    ...
    if isinstance(callback.message, Message):
        with suppress(TelegramAPIError):
            await callback.message.edit_text(
                render_key_detail(blueprint, True, provider.config.masked_key, health_line),
...
async def on_keys_clear(
    ...
    ai.reload(_build_providers(store))
...
async def main() -> None:
    ...
    settings = Settings.load(key_store)
